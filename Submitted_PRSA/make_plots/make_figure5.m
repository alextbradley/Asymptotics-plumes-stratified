...
%figpref(4)
%% parameters
run parameters/typical_parameters.m
...
%variable scalings
chi_scaling = lambda^(-1/4)*Qc^(1/2)*dzb_xc^(-1/2);
u_scaling = lambda^(1/4)*dzb_xc*Qc^(1/2);
p_scaling = lambda^(3/4)*dzb_xc*Qc^(1/2);
...
U_ff        = (-chi_ff)^(1/3)*dzb_xc^(2/3)*Qc^(1/3)*lambda^(1/3) ;
D_ff        = (-chi_ff)^(-1/3)*dzb_xc^(-2/3)*Qc^(2/3)*lambda^(-1/3);
delta_P_ff  = -chi_ff*dzb_xc*lambda;
delta_T_ff  = -(-chi_ff)^(1/3)*Qc^(2/3)*dzb_xc^(1/3) * lambda^(-1/3);
...
M1   = @(chi,Y) mass1(chi,Y, k2, eps1);
rhs1 = @(chi,Y) forcing1(chi,Y,eps1,eps4, lambda, Pt,k3,zbF, dzbF,Xc);
...
M2   = @(chi,Y) mass2(chi,Y, k2, lambda);
...
%% functions
function M = mass1(chi,Y, k2, eps1)
...
function f = forcing1(chi,Y,eps1,eps4, lambda, Pt,k3,zbF, dzbF,Xc)
